...
app_state: web::Data<app_state::AppState>,
 ...
 use crate::utils::mailer::YandexSmtpClient;
    ...
    // Когда код дергал ты??
    if !time_examin {
        return Err(ApiResponse::new(409, "Истекло время кода".to_string()));
    }

    // Проверяем email на пустоту
    if !null_email(&email_json.email) {
        return Err(ApiResponse::new(409, "Пустой Email".to_string()));
    }
    ...
    return Err(ApiResponse::new(409, "Не существующий Email".to_string()));
        }
...
-> Result<ApiResponse,ApiResponse> {
    ...
    user_logins_active.token = sea_orm::ActiveValue::Set(Some(token.clone()));
